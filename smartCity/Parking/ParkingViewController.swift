...
class ParkingViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if model.parkingId != nil{
            ...
            let vc = UIStoryboard.init(name: "Parking", bundle: Bundle.main).instantiateViewController(withIdentifier: "ParkingDetailViewController") as! ParkingDetailViewController
    ...
    @IBAction func filterDataButtonClick(_ sender: UIButton) {
        ...
        for button in filterBtnArray{
            if button == sender{
                ...
                if button.tag == 10{
                    weak var weakSelf = self
                    filterView = FilterView.init(frame: CGRect.init(x: 0, y: 44 + 65, width: SCREEN_width, height: SCREEN_height-44), titleArray: ["综合排序","停车价格最低","充电价格最低","免费停车","24小时"], lastSelected: filterLastIndex)
