...
//  smartCity
//
//  Created by George on 2018/4/22.
...
final class ParkingDatasource: NSObject {
    ...
    class Condition{
        ...
        var sort:Int = 0 // 0 默认综合 1停车价格最低，2充电价格最低，3免费停车，4 24小时，5停车场空闲优先，6停车场距离最近
        var keyword:String = ""
    }
    ...
    @objc dynamic var datasourceHaveBeenChanged:Bool = false
    
    
    var islast = false //判断是否是最后一条数据
    var isUpdate = false //防止重复加载
    var currentPage = 1
    func getData(view:UIView){
        ...
        handle.httpPostRequest(url: ApiManager.ParkingApi.parkingNumListAddress, params: param) { (success, result, _) in
            if success{
                ...
                weakSelf?.data = array
                weakSelf?.datasourceHaveBeenChanged = true
            }
        }
    }
    //上拉加载更多
