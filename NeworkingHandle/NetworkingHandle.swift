...
class NetWorkHandle:NSObject{
    ...
    func httpRequestWith(url:String,params:[String:Any]?,requstType:RequestType,isCache:Bool,cacheKey:String?,imageData:[(imageName:String,fileName:String,image:UIImage)]?,finish:@escaping (_ succeed:Bool,_ result:Any?,_ code:Int?) ->Void){
        ...
        if showToast == true && currentView != nil{
            HCZProgressHUD.shared.show(currentView: currentView!)
        }
        var cacheData:Any?
        if isCache == true{
        ...
        if requestData != nil{
            if requestData is NSDictionary{
                ...
                if jsonModel.code == 0{
                    ...
                    if currentView != nil{
                        HCZMessage.show(message: jsonModel.msg == "" ? "数据异常": "\(jsonModel.msg)", window: currentView!.window)
                    }
                    finish(false,jsonModel.msg,jsonModel.code)
                }
            }else{
                if currentView != nil{
                    HCZMessage.show(message:  "数据格式错误", window: currentView!.window)
                }
    ...
    func showErrorMsg(error:Error){
        ...
        HCZMessage.show(message:  "连接服务器失败,错误提示：\(error.localizedDescription)", window: self.currentView!.window)
